...
@app.callback(
    ...
    Input({'type':'n_SaB','index':ALL},'value'),
...
def generateOutput(n_clicks1,n_clicks2, resultSort, clickData,
                    I_x_values, n_SaB_values, c_main_values,
    ...
    similar_prod_info_methods = pd.DataFrame(list(zip(I_x_values,n_SaB_values,c_main_values)), columns=["I_x","n_SaB","c_main"])
    ...
    if SgB ==0:
        I_al=KW_IiP[0].investition
        best_method_3 = KW_IiP[0].name
        ...
        KW_IiP=[x.table for x in KW_IiP]
    else:
    ...
    if n_clicks2>0:
        returns[2:]=[KW_l2,KW_l3,KW_IiP,KW_KäP, f"Optimal: {best_method}", f"Optimal: {best_method_3}", f"Optimal: {best_method_l2}", f"Optimal: {best_method_l3}"]
