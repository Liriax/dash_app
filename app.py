                        ...
                        children=[
                                ...
                                children=[
                                    ...
                                    html.Td(colSpan=2,
                                        ...
                                        dcc.Dropdown(
                                            id='calMethod',
                                            options=[
                                                {'label': 'Statische Kostenvergleichsrechnung', 'value': 'staCost'},
                                                {'label': 'Dynamische Investitionsrechnung mit Kapitalwertmethode', 'value': 'NPV'}
                                            ],
                                            value='staCost'    
                                            )
                                    ])
                            ...
                            html.Tr(id="npvChainOutput",
                                    children=[
                                        ...
                                        html.Td(children=[dcc.Input(id='npvRevProProduct', type = 'number', min=0, value=500)]),
                    ...
                    children=[
                        html.Table(
                                children=[
                                    ...
                                    html.Tr(
                                        children=[
                                            html.Th(colSpan = 3, style = {'text-align':'left'}, children=["Option {}".format(opt)]),                                                    ]
                                    ),
                                    # row 3
                                    html.Tr(
                                        children=[
...
def save_ist_situation(n_clicks, matLevel, supFunction, t1,f1,t2,f2,t3,f3,t4,f4,t5,f5,t6,f6,t7,f7,t8,f8,t9,f9,numNewVariant):
    total_search_time = t1*f1+t2*f2+t3*f3+t4*f4+t5*f5+t6*f6+t7*f7+t8*f8+t9*f9
    data = {'matLevel':matLevel, 'supFunction':supFunction, 'totalSearchTime': total_search_time, 'numNewVariant': numNewVariant}
...
def save_parameter_Investitionsrechnung(n_clicks,I_al,I_pr,I_l2,I_l3,C_person,C_main,C_int,t,npvRevProProduct):
    data={'I_al': I_al, 'I_pr':I_pr, 'I_l2':I_l2, 'I_l3':I_l3, 'C_person':C_person, 'C_main': C_main, 'C_int': C_int, 't':t, 'npvRevProProduct':npvRevProProduct}
    df = pd.DataFrame([data])
